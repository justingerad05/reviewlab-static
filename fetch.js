...
link.addEventListener("touchstart",()=>{touchTimer=setTimeout(()=>{
hover.src=img.dataset.src;hover.style.display="block";hover.style.top="40%";hover.style.left="50%";hover.style.transform="translate(-50%,-50%)";},350);});
...
/* =========================
PHASE 32 — TOPICS PAGES (FIX 404) ======================== */

const topicList = ["review", "suite"];
topicList.forEach(topic => {
  const topicPosts = posts.filter(p => p.category === topic);
  const topicHtml = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${topic.charAt(0).toUpperCase() + topic.slice(1)} Articles</title>
<link rel="canonical" href="${SITE_URL}/topics/${topic}.html">
<meta name="description" content="All ${topic} articles on ReviewLab">
<style>
body{font-family:system-ui;max-width:900px;margin:auto;padding:40px;line-height:1.7;background:#020617;color:#e5e7eb;}
...
<ul>
${topicPosts.map(p=>`<li><a href="${p.url}">${p.title} (~${p.readTime} min)</a></li>`).join("")}
</ul>
</body>
</html>`;
  fs.writeFileSync(`topics/${topic}.html`, topicHtml);
});

/* =========================
PHASE 33 — AUTHOR PAGE ======================== */
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Justin Gerald – ReviewLab Author</title>
<link rel="canonical" href="${SITE_URL}/author/">
<meta name="description" content="All posts by Justin Gerald on ReviewLab">
<style>
body{font-family:system-ui;max-width:900px;margin:auto;padding:40px;line-height:1.7;background:#020617;color:#e5e7eb;}
...
<body>
<h1>Posts by Justin Gerald</h1>
<ul>
${posts.map(p=>`<li><a href="${p.url}">${p.title} (~${p.readTime} min)</a> - ${p.category}</li>`).join("")}
</ul>
</body>
</html>`;
...
/* =========================
PHASE 31 — PROGRAMMATIC COMPARISONS (UNCHANGED) ======================== */
...
console.log("✅ FETCH COMPLETE — POSTS, TOPICS, AUTHOR, COMPARISONS, SITEMAP BUILT");
