...
for (const entry of entries) {
  ...
  posts.push({ title, slug, html: rawHtml, url, description, og: primaryOG, thumb, readTime, date: entry.published, category: primaryCategory, schemas: JSON.stringify([articleSchema, breadcrumbSchema, reviewSchema, organizationSchema]) });
}
...
/* =========================
   TOPIC PAGES
========================= */
const allTopics = ["suite", "review", ...new Set(posts.map((p) => p.category))];
for (const topic of allTopics) {
  const topicPosts = posts.filter((p) => p.category === topic);
  const listItems = topicPosts
...
<ul>${listItems}</ul>
  ...
  fs.writeFileSync(`topics/${topic}.html`, page);
}
...
/* =========================
   COMPARISONS, JSON, SITEMAP
   (unchanged)
========================= */

console.log("✅ POSTS + TOPICS + AUTHOR BUILT SUCCESSFULLY — suite/review fixed");
